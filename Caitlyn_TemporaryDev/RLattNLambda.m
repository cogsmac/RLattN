...
function actionsMade = RLattNLambda(experimentStruc, params)
...
for blockTrial = 1:length(stimuli)
    ...
    while action <= featureMax
        ...
        if action <= featureMax % fixations are 1-3
            ...
            tdError = lambda*V(currentState) + reinforcement - V(previousState) ;
            V(previousState) = V(previousState) + lambda*(tdError)         ;
